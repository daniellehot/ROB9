...
img_folder = cwd + '/img'
img_save_folder = cwd + '/img_save'
print("img_folder: ", img_folder)
print("img_save_folder: ", img_save_folder)
...
def visualize_mask(im, rois_final, rois_class_score, rois_class_ind, masks, ori_height, ori_width, im_name, thresh):
    ...
    for i in xrange(0, num_boxes):
        ...
        if cfg.TEST.MASK_REG:
            ...
            cv2.imwrite(img_save_folder + '/' + im_name.split('.')[0] + '_mask'+str(i)+'.png', color_curr_mask)


    ori_file_path = img_folder + '/' + im_name.replace("rgbd.jpg", "rgb.jpg")
    ...
    cv2.imwrite(img_save_folder + '/' + im_name.split('.')[0] + '_obj_detction.png', img_out)
    cv2.waitKey(0)
...
if __name__ == '__main__':
    ...
    caffemodel = root_path + '/pretrained/vgg16_faster_rcnn_iter_110000_pam_7attribute.caffemodel'
