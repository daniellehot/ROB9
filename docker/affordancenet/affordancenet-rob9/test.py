...
def im_detect(net, im, boxes=None):
    ...
    print("blobs and im_scales: ", blobs, im_scales)
    ...
    if cfg.DEDUP_BOXES > 0 and not cfg.TEST.HAS_RPN:
        print("cfg.DEDUP_BOXES > 0 and not cfg.TEST.HAS_RPN TRUE")
        ...
        boxes = boxes[index, :]
        print("cfg.DEDUP_BOXES > 0 and not cfg.TEST.HAS_RPN TRUE")
        print("cfg.Dedup_boxes > 0 and some other sutff made it through ", boxes)
    ...
    print("on the otherside of forward_kwargs")
    blobs_out = net.forward(**forward_kwargs)
    print("on the otherside of net.forward")
...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in xrange (num_images):
        ...
        for nb in xrange(0, len(rois_class_ind)):
            c = int(rois_class_ind[nb,0])
            if c < 0:
                continue
            ...
            w = x2 - x1 + 1
            mask = masks_out[nb, :, :]
            mask = cv2.resize(mask.astype(np.float32), (int(w), int(h)), interpolation=cv2.INTER_LINEAR)
            mask[mask >= 0.5] = 1
            mask[mask < 0.5] = 0
